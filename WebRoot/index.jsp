...
int i,nouse=0,favor=0;
  ...
  Evaluate evaluate;
  String title,writer,time,pic,content,md,contsub,rtitle,rmd,stitle,smd;
  Integer scan,count=0,commcount=0,collcount=0; 
  Collection collection;
  %>
<%
if(messlist==null){
request.getRequestDispatcher("InitMess?url=index.jsp").forward(request, response);
}%>
...
<head>
 ...
 <script src="${pageContext.request.contextPath}/js/sliders.js"></script>
		      ...
		      <figure>
			   ...
			   <ul>
			     ...
			     <p class="autor2">
				  ...
				  <p><%=contsub %>..."</p>
			     <p class="autor">
					 <span class="dislike f_r"><a href="EDetailMess?value=nouse&md=<%=md%>&url=<%="index.jsp" %>">踩（<%=nouse %>）</a></span>
					 <span class="like f_r"><a href="EDetailMess?value=favor&md=<%=md%>&url=<%="index.jsp" %>">喜欢（<%=favor %>）</a></span>
					 <span class="collect f_r"><a href="Collect?title=<%=title%>&writer=<%=writer%>&md=<%=md%>&url=<%="index.jsp"%>">收藏（<%=collcount %>）</a></span>
