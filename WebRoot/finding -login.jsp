...
<%!
  ...
  Mess mess;
  Evaluate evaluate;
  String title,writer,time,pic,content,md,userId,avatar,contsub;
  Integer scan,collcount=0,commcount=0;  
  %>
<% if(messlist==null){
request.getRequestDispatcher("InitMess?url=finding -login.jsp").forward(request, response);
return;
}  
			   ...
			   <ul>
			     ...
			     <p class="autor">
				    ...
				    <span class="lm f_l">
					 ...
					 -->
					 <span class="dislike f_r"><a href="EDetailMess?value=nouse&md=<%=md%>&url=<%="finding -login.jsp"%>">踩（<%=nouse %>）</a></span>
					 <span class="like f_r"><a href="EDetailMess?value=favor&md=<%=md%>&url=<%="finding -login.jsp"%>">喜欢（<%=favor %>）</a></span>
					 <span class="collect f_r"><a href="Collect?title=<%=title %>&writer=<%=writer %>&md=<%=md %>&url=<%="finding -login.jsp" %>">收藏（<%=collcount %>）</a></span>
